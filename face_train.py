...
class Dataset:
    ...
    def load(self, img_rows=IMAGE_SIZE, img_cols=IMAGE_SIZE,
        ...
        if K.image_data_format() == 'channels_first':
            ...
            类别标签进行one-hot编码使其向量化，在这里我经过转化后标签数据变为二维们的类别只有两种，
            '''
...
class Model:
    ...
    def train(self, dataset, batch_size=20, nb_epoch=10, data_augmentation=True):
        ...
        # 不使用数据提升，所谓的提升就是从提供的训练数据中利用旋转、翻转、加噪声等方法创造新的
    ...
    def face_predict(self, image):
        # 依然是根据后端系统确定维度顺序
        if K.image_data_format() == 'channels_first' and image.shape != (1, 3, IMAGE_SIZE, IMAGE_SIZE):#把 K.image_dim_ordering() == 'th' 修改为K.image_data_format() == 'channels_first'
            image = resize_image(image)  # 尺寸必须与训练集一致都应该是IMAGE_SIZE x IMAGE_SIZE
            image = image.reshape((1, 3, IMAGE_SIZE, IMAGE_SIZE))  # 与模型训练不同，这次只是针对1张图片进行预测
        elif K.image_data_format() == 'channels_last' and image.shape != (1, IMAGE_SIZE, IMAGE_SIZE, 3):#把K.image_dim_ordering() == 'tf' 修改为 K.image_data_format() == 'channels_last'
...
if __name__ == '__main__':
    dataset = Dataset('D:/MYDESIGN/TEST414/data/')
    dataset.load()
    model = Model()
